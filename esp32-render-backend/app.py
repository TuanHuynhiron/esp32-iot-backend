...
def safe_base64_decode(b64_str)
    Xử lý và giải mã Base64 an toàn, tự sửa lỗi paddingxuống dòng từ SIM 4G
    clean_b64 = b64_str.strip().replace( , +).replace(r, ).replace(n, )
    mod = len(clean_b64) % 4
    if mod == 1
...
@app.route('staticuploadspathfilename')
def serve_upload(filename)
    return send_from_directory(UPLOAD_FOLDER, filename)

# 1. API CẤP THỜI GIAN ĐỒNG BỘ RTC CHO ESP32
@app.route('apiget-time', methods=['GET'])
def get_time()
    now_str = datetime.now().strftime(%Y%m%d_%H%M%S)
    return now_str, 200, {'Content-Type' 'textplain'}
...
def get_data()
    ...
    try
        ...
        data = []
        for r in rows
            # Chuẩn hóa tên file ảnh (chỉ lưu tên file trong DB để giao diện Web ghép đường dẫn sạch)
...
# 3. API NHẬN CHUNK DỮ LIỆU & GHÉP ẢNH TỪ ESP32 VIA SIM 4G
@app.route('apidata-chunk', methods=['POST'])
...
@app.route('apideleteintrecord_id', methods=['DELETE'])
...
@app.route('apithresholds', methods=['POST'])
def set_thresholds()
...
if __name__ == '__main__'
    # Đọc cổng động do Render cấp qua os.environ
